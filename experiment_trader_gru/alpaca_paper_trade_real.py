...
def sell(symbol, shares_owned, trade, capital, slippage=.005):
    ...
    if shares_to_sell > 0:
        ...
        logging.info(dict(
            type='no_trade_sell',
            message=f'Selling 0 shares of {symbol}',
            payload=dict(
                trade=trade,
                capital=bar_state[symbol][CAPITAL_KEY],
                shares_owned=bar_state[symbol][SHARES_OWNED_KEY],
...
def buy(symbol, trade, capital, slippage=.005):
    ...
    if shares_to_buy > 0:
        ...
        logging.info(dict(
            type='no_trade_buy',
            message=f'Buying 0 shares of {symbol}',
            payload=dict(
                trade=trade,
                capital=bar_state[symbol][CAPITAL_KEY],
                shares_owned=bar_state[symbol][SHARES_OWNED_KEY],
...
def main():
    ...
    ############
    """
    Instantiate barstate of the gapped up symbols
    """
    end_time = get_current_datetime()
    # TODO: Get the previous market open, not yesterday
    # TODO: I thought I only need data from market open today???
    ...
    ###############
    """
    Keep filling the barstate until market is open 
    """
    ...
    ###############
    """
    The event loop starting process for each symbol
    """
