...
class PercentChangeNormalizer:
    ...
    def normalize_volume(cls, data, dataset_name):
        ...
        for batch_index in range(len(data)):
            ...
            current_batch_data[:, VOLUME_COLUMN_INDEX] -= stats[dataset_name]['volume']['mean']
            current_batch_data[:, VOLUME_COLUMN_INDEX] /= stats[dataset_name]['volume']['std']
        return torch.Tensor(data)

    @classmethod
    def normalize_price_into_percent_change(cls, _data):
        data = np.copy(_data)
        for batch_index in range(len(data)):
            current_batch_data = data[batch_index]
